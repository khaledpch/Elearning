...
<html lang="en">
  <head>
    ...
    <title>Consulter apprenants</title>
...
<section id="main-content" align="center">
          <section class="wrapper">
              ...
              <div class="row">
                  <div class="col-sm-6">
                      <section class="panel">
   <center> <h1>liste des apprenant</h1></center>
</div>
...
<table width="755">
	<tr>
        ...
        <td>
        	<a href="ajout.php"><input type="button" title="Add new student" name="butAdd" class="btn btn-danger" value="Ajouter apprenant" id="button-search" /></a>
        </td>
        <td >
                           ...
                           <tbody>
							  ...
							  <th name="id_app"><i class="icon_calendar"></i> numero</th>
								 ...
								 <th><i class="icon_calendar"></i> date </th>
								   ...
								   <th><i class="icon_calendar"></i> nivo </th>
...
$key="";
	...
	if($key !="")
		$sql_sel=mysql_query("SELECT * FROM ` apprenants` WHERE `Nom` ='$'");
		else
		 $sql_sel=mysql_query("SELECT * FROM ` apprenants`");
	
		
       
    $i=0;
$result = mysql_query("SELECT  `id_apprenant`,`pseudo`, `password`, 	CONCAT(`Nom`,' ', `prenom`) as np, `ville`, `email` ,`date_naiss`, `niveau_app` FROM ` apprenants` ORDER BY np");
$nblignes = mysql_numrows($result);
...
for ($i=0;$i<$nblignes;$i=$i+1) {
	 ...
	 $user_pseudo = mysql_result($result,$i,"pseudo");
	  $user_pass = mysql_result($result,$i,"password");
	 ...
	 echo "<tr><td>$user_id</td><td>$user_pseudo</td><td>$user_pass</td><td>$user_prenom</td><td>$user_ville</td><td>$user_email</td><td>$user_date</td><td>$user_nivo</td><td>  <div class=\"btn-group\">" ?>
                              <a class ="delete" href="delete.php?id_app=<?php echo $user_id ?>"><button class="btn btn-danger" title="supprimer" ><i class="icon_close_alt2"></i></button></a>
							  <a class ="" href="moddif_app.php?id_app=<?php echo $user_id ?>"><button class="" title="modifier" ><i class="icon_plus_alt2"></i></a></button></a>
